...
class PatchGAN(L.LightningModule):
    ...
    def load_last_checkpoint(self, checkpoint_path):
        checkpoints = sorted(glob.glob(os.path.join(checkpoint_path, "patchgan_*.pth")))

        epochs = set([
            int(ch.split('/')[-1].replace('patchgan_', '')[:-4]) for ch in checkpoints
        ])

        try:
            assert len(epochs) > 0, "No checkpoints found!"
            start = max(epochs)
            checkpoint_file = os.path.join(checkpoint_path, f"patchgan_{start:03d}.pth")
            print(f"Loading from {checkpoint_file}")
            return self.load_from_checkpoint(checkpoint_file)
